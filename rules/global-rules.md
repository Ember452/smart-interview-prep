...
执行规则：
- **追问决策必填**：类型必须从 `rules/interview-process.md` 追问策略映射表选择（全链路讲解/难点前置/选型逼问/边界异常/量化逼问/bad case 闭环/挑战式质疑/行业对标/复盘反思/规模必要性质疑/降级追问/名词抓取/切换话题），依据必须对应回答中的具体信号；无匹配信号时默认「名词抓取」或「切换话题」
