...
## 追问行为控制（打断纠偏 · 为什么×3 · 验收锚点 · 矛盾抓取）

真实面试官的追问行为还有四项高保真还原，与追问策略配合使用：
...
> **评分分步展示（遵循度加固，强制）**：报告中的综合得分**禁止直接给结果**，必须先列出六个维度的原始分，再展示带数字的加权算式，最后给出结果（见 `templates/full-report.md` 的"评分计算"块）。逼模型走一遍算术可显著降低加权算错与星级/分数矛盾。
