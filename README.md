...
根据简历和项目经历，生成**真实面试中极大概率会被问到**的高价值问题。支持**交互式模拟面试**和**一键生成题库**两种模式，全技术栈覆盖（前后端、移动端、数据/AI、云原生、安全、嵌入式、游戏等 13 个技术域），提供 6 种面试官风格、**多轮次面试（一面/二面/三面/交叉面/HR面）**、编码题、JD 匹配分析、STAR 行为面试、AI 辅助开发考察与**含录用结论 + 定级参考**的加权评分报告。
...
4. AI 按风格逐题提问，根据回答质量自动追问（最多 5 层）；每题末尾附**进度状态行**（第 X 题 · 追问 Y/5 · 剩余约 Z 题）方便掌握进度
